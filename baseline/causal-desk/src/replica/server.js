...
// 按根暂扣只在主侧消费台按/解；副本只读镜像暂扣状态。非消费工在主侧本就是 403，这里同样先按角色拒
const holdStub = (req, _res, next) => {
  if (req.user.role === 'consume') {
    return next(new HttpError(405, 'REPLICA_READ_ONLY', '按根暂扣在主侧消费台按/解，副本只读镜像暂扣状态'));
  }
  return next(forbidden('ROLE_DENIED', '只有消费工可以按/解按根暂扣'));
};
app.post('/api/holds', auth, holdStub);
app.delete('/api/holds/:root', auth, holdStub);
...
// 搁置/解开同根并发对只在主侧对账台进行；副本只读镜像队列状态（含搁置标记，随 /api/internal/sync）
const shelveStub = (req, _res, next) => {
  if (req.user.role === 'reconcile') {
    return next(new HttpError(405, 'REPLICA_READ_ONLY', '搁置/解开并发对只在落库台主侧对账台进行，副本只读'));
  }
  return next(forbidden('ROLE_DENIED', '只有对账工可以搁置/解开同根并发对'));
};
app.post('/api/queue/:id/shelve', auth, shelveStub);
app.delete('/api/queue/:id/shelve', auth, shelveStub);
