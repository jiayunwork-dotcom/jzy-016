...
const initialState = () => ({
  counters: { seq: 0, batch: 0, abort: 0, queue: 0, snapshot: 0, seal: 0, hold: 0, shelve: 0 },
  channelUp: true,
  users: [],
  events: [],
  queue: [],
...
async function migrate() {
  await store.mutate((draft) => {
    draft.seals ??= {};
    draft.counters.seal ??= 0;
    draft.holds ??= {};
    draft.counters.hold ??= 0;
    // 旧版本并发对没有搁置字段：补齐为"没搁"（搁置状态本来就挂在队列对上，随队列入库）
    draft.counters.shelve ??= 0;
    ...
    // 旧游标是连续前缀模型：补记已放行集合 1..lastSeq（已放到的不改口）
    for (const c of Object.values(draft.cursors ?? {})) {
      if (!Array.isArray(c.deliveredSeqs)) {
        c.deliveredSeqs = Array.from({ length: c.lastSeq ?? 0 }, (_, i) => i + 1);
      }
    }
  });
}
...
function publicView() {
  ...
  return {
    ...
    counts: {
      ...
      queueOpen: d.queue.filter((q) => !q.verdict).length,
      queueDecided: d.queue.filter((q) => q.verdict).length,
      queueShelved: d.queue.filter((q) => q.shelveId).length
    },
    ...
    seals: d.seals ?? {},
    holds: d.holds ?? {},
    releasedSeqs: [...releasedSeqs(d)].sort((a, b) => a - b)
  };
}
...
// ---- 停写封条（仅落库工）：封的是聚合根，不是通道 ----
// 通道可以开着，被封的根整批提交被 409 拦下；在库旧事件照常消费/裁决/导出。
app.get('/api/seals', auth, (_req, res) => res.json({ seals: store.read().seals ?? {} }));
app.post('/api/seals', auth, requireRole('ingest'), async (req, res, next) => {
  try {
    assertCanSeal(req.user); // 兜底：后台也拒
    const seal = await store.mutate((draft) => sealRoot(draft, req.body?.root, req.user));
    res.status(201).json({ ok: true, seal, note: `聚合根 ${seal.root} 已停写：通道照样开着，其他根照常追加` });
  } catch (e) { next(e); }
});
app.delete('/api/seals/:root', auth, requireRole('ingest'), async (req, res, next) => {
  try {
    assertCanSeal(req.user); // 兜底：后台也拒
    const r = await store.mutate((draft) => unsealRoot(draft, req.params.root, req.user));
    res.json({ ok: true, ...r, note: '封条已揭：该根可再交新事件；此前被拦的批次不会偷偷补进' });
  } catch (e) { next(e); }
});

// ---- 按根暂扣（仅消费工）：挡的是放行，不是入库 ----
// 被扣的根还没放到的事件不再放行（别的根照常拉动）；该根照常追加，导出仍只带已经放到的。
app.get('/api/holds', auth, (_req, res) => res.json({ holds: store.read().holds ?? {} }));
app.post('/api/holds', auth, requireRole('consume'), async (req, res, next) => {
  try {
    assertCanHold(req.user); // 兜底：后台也拒
    const hold = await store.mutate((draft) => holdRoot(draft, req.body?.root, req.user));
    res.status(201).json({ ok: true, hold, note: `聚合根 ${hold.root} 已暂扣：未放到的不再放行（别的根照常拉动）；入库不挡，仍可追加` });
  } catch (e) { next(e); }
});
app.delete('/api/holds/:root', auth, requireRole('consume'), async (req, res, next) => {
  try {
    assertCanHold(req.user); // 兜底：后台也拒
    const r = await store.mutate((draft) => unholdRoot(draft, req.params.root, req.user));
    res.json({ ok: true, ...r, note: '暂扣已解：该根从第一条未放出的事件继续放行；已经放到的不改口' });
  } catch (e) { next(e); }
});
...
// ---- 搁置 / 解开同根并发对（仅对账工）：搁的是裁决，不是入库/拉动 ----
// 搁置期间这一对不能选胜负（adjudicate 返回 409 PAIR_SHELVED），旁边没搁置的对照常裁；
// 入库（appendBatch）和拉动（pullPrimary）完全不查搁置；导出照旧不带未决对及后继。
// 落库工/消费工页面没有这枚钮，后台 requireRole + assertCanShelve 一样拒；没登录 401。
app.post('/api/queue/:id/shelve', auth, requireRole('reconcile'), async (req, res, next) => {
  try {
    assertCanShelve(req.user); // 兜底：后台也拒
    const q = await store.mutate((draft) => shelvePair(draft, req.params.id, req.user));
    res.status(201).json({
      ok: true,
      queue: q,
      note: `并发对 ${q.id} 已搁置（${q.shelveId}）：搁着不能选胜负；导出照旧不带这两条及后继；入库、拉动不受影响；解开后才能再裁`
    });
  } catch (e) { next(e); }
});
app.delete('/api/queue/:id/shelve', auth, requireRole('reconcile'), async (req, res, next) => {
  try {
    assertCanShelve(req.user); // 兜底：后台也拒
    const r = await store.mutate((draft) => unshelvePair(draft, req.params.id, req.user));
    res.json({
      ok: true,
      ...r,
      note: '搁置已解开：这一对可以再裁；已裁过的对不会因为解开而改判'
    });
  } catch (e) { next(e); }
});
...
app.get('/api/rbac/matrix', auth, (_req, res) => {
  res.json({
    ...
    rules: {
      ...
      'POST /api/seals': ['ingest'],
      'DELETE /api/seals/:root': ['ingest'],
      'POST /api/snapshots': ['ingest'],
      'GET /api/export': ['ingest'],
      'POST /api/queue/:id/adjudicate': ['reconcile'],
      'POST /api/queue/:id/shelve': ['reconcile'],
      'DELETE /api/queue/:id/shelve': ['reconcile'],
      'POST /api/consume/pull': ['consume'],
      'GET /api/consume/cursors': ['consume'],
      'POST /api/holds': ['consume'],
      'DELETE /api/holds/:root': ['consume']
    }
  });
});
