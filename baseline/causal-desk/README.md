...
同一聚合根上的事件按**向量时钟因果序**落库：声明的前驱没在库、或同根两笔时钟对不上，就不能追加；消费侧后继到了、前驱没齐，就不放行。三个工位（落库台 / 消费台 / 对账台）+ 主/副本，Docker Compose 一键启动。
...
| 账号 | 密码 | 工位 | 能干 | 不能干 |
| --- | --- | --- | --- | --- |
| `ingest` | `ing123` | 落库工 | 追加事件/声明前驱/整批提交、通道开关、快照、导出 | 裁决、拉游标 |
| `consume` | `con123` | 消费工 | 主侧/副本拉动消费、看游标 | 追加、裁决、改同步模式 |
| `reconcile` | `rec123` | 对账工 | 同根并发裁决、只读在库 | 追加业务事件、动游标、改前驱 |
...
| 需求规则 | 实现 |
| --- | --- |
| 追加同时满足：声明前驱每条已在库（或本批更早）、本批对外依赖齐 | `src/primary/causal.js` `appendBatch()` 第 2 步，缺前驱返回 `PREDECESSOR_MISSING` |
...
| 批内前驱 DAG 校验、拓扑序提交（前驱 seq 必更小） | `appendBatch()` 第 3–6 步，有环返回 `CYCLE_IN_BATCH` |
...
3. **断开通道**再追加 → 409；恢复。
4. **退出，登录 `reconcile`**：打开对账台，把两对并发逐一裁决（A/B 选边）；看落库台「导出」从挡 4 条变成全量可导出。
5. **登录 `consume`**：主侧拉动，游标逐次前进到底。
6. 消费台副本区（故障注入按钮只有 `ingest` 能按，消费工按会收到 403——可回 `ingest` 操作）：
...
7. 任意工位底部「权限自检」：所有越权动作都应显示 `403 ✓` / `405 ✓`。
