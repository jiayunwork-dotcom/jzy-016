...
同一聚合根上的事件按**向量时钟因果序**落库：声明的前驱没在库、或同根两笔时钟对不上，就不能追加；消费侧后继到了、前驱没齐，就不放行。落库工还能给在库聚合根贴**停写封条**：封的是根、不是通道——通道可以开着，被封的根整批提交照样被拦；在库旧事件照常消费、裁决、导出。消费工能在消费台按**按根暂扣**：扣的是放行、不是入库——被扣的根还没放到的事件不再放行（拉动停在该根第一条未放出的事件前，不越过它装后面的；别的根照常拉动），但这根照常追加，导出仍只带已经放到的。对账工能在对账台对未决同根并发按**搁置**：搁的是这一对的裁决、不是入库/拉动——搁着的对不能选胜负，导出照旧不带这两条和顺着它们的后继，旁边没搁置的对还能裁；这对所在的根还能追加，已经放到的事件照常放行，解开之后才能再裁这一对。三个工位（落库台 / 消费台 / 对账台）+ 主/副本，Docker Compose 一键启动。
...
| 账号 | 密码 | 工位 | 能干 | 不能干 |
| --- | --- | --- | --- | --- |
| `ingest` | `ing123` | 落库工 | 追加事件/声明前驱/整批提交、通道开关、贴/揭停写封条、快照、导出 | 裁决、拉游标、按/解暂扣 |
| `consume` | `con123` | 消费工 | 主侧/副本拉动消费、看游标、按/解按根暂扣 | 追加、裁决、贴/揭封条、改同步模式 |
| `reconcile` | `rec123` | 对账工 | 同根并发裁决、搁置/解开未决并发对、只读在库 | 追加业务事件、动游标、改前驱、贴/揭封条、按/解暂扣 |
...
| 按根暂扣：被扣的根还没放到的事件不再放行，拉动停在该根第一条未放出的事件前、不越过它装后面的；别的根照常拉动 | `pullPrimary()` / `pullReplica()`：游标从"连续前缀"升级为"已放行集合"（`deliveredSeqs`），被扣的根整根跳过（只挡这根的放行），`heldBack` 报出停在哪条前；连续前缀水位随之停在该根第一条未放出的事件前 |
| 暂扣只挡放行，不挡入库：被扣的根照常追加；导出仍只带已经放到的 | `appendBatch()` 不查暂扣；`buildExport()` 对被扣的根只放"已放到"（主侧所有消费者游标的并集）的事件，未放出的记 `ROOT_HELD_NOT_RELEASED` 挡住，后继沿前驱传递挡住 |
| 同一根同时只能有一份暂扣；还没在库的根不能扣 | `holdRoot()`：重复扣 409 `HOLD_EXISTS`，不在库 404 `ROOT_NOT_IN_STORE`；`holds` 以根为键，结构上就放不下第二份 |
| 只有消费工能按/解；落库工、对账工页面没入口，后台一样拒；没登录动不了 | `POST /api/holds`、`DELETE /api/holds/:root` 挂 `requireRole('consume')` + `assertCanHold()` 兜底，越权 403；所有暂扣路由挂 `requireAuth`，无 token 401 |
| 解开之后该根才能继续放行；解开之前已经放到的不改口 | `unholdRoot()`；已放行集合只增不减、游标绝不回退，导出对"已放到"的口径不变；解扣后从该根第一条未放出的事件续放 |
| 暂扣状态镜像到副本：副本拉动同样拦住被扣的根 | `/api/internal/sync` 负载带 `holds`，副本 `syncOnce()` 镜像、`pullReplica()` 同口径拦截；副本上按/解暂扣 405 只读 |
| 搁置未决并发对：搁的是这一对的裁决，不是入库/拉动；搁着不能选胜负 | 对账台每对未决并发旁边一枚搁置；`shelvePair()` 只在队列对上挂 `shelveId`，`adjudicate()` 见搁置返回 409 `PAIR_SHELVED`；`appendBatch()` / `pullPrimary()` / `pullReplica()` 完全不查搁置 |
| 搁置期间导出照旧不带这两条和顺着它们的后继；旁边没搁置的对还能裁 | 搁置不改 `verdict`（仍是未决对），`buildExport()` 的未决对两侧 + 前驱传递闭包照旧挡住；导出 `shelves` 字段列出搁置中对；其他对的裁决路径不看搁置 |
| 入库和拉动不受影响：这对所在的根还能追加，已经放到的事件还能放行 | `appendBatch()` 不查搁置（新后继照入库），`pullPrimary()`/`pullReplica()` 只看前驱可见性与暂扣，搁置对两侧事件照常放到 |
| 一对未决同时只能搁一份；已经有裁决结果的对不能搁置 | `shelvePair()`：重复搁 409 `ALREADY_SHELVED`，已裁决 409 `ALREADY_DECIDED`（搁置按钮只对未决对出现，已裁对不出现），不存在 404 |
| 解开之后才能再裁这一对；已裁过的对不要因为解开就改判 | `unshelvePair()` 只清 `shelveId/shelvedBy*`，绝不碰 `verdict/winnerEvent/decided*`；没搁置 409 `NOT_SHELVED`；改判仍由 `ALREADY_DECIDED` 409 拦 |
| 只有对账工能搁置/解开；落库工、消费工工位没有这枚钮，接口也拒；没登录动不了 | `POST/DELETE /api/queue/:id/shelve` 挂 `requireRole('reconcile')` + `assertCanShelve()` 兜底，越权 403，无 token 401；搁置状态随 `queue` 镜像到副本，副本上搁置/解开 405 只读 |
...
5. **退出，登录 `reconcile`**：打开对账台，每对未决并发旁边有一枚「搁置」。先把某一对按**搁置**：这一对挪到「搁置中」，胜负按钮消失不能裁；落库台「导出」照旧挡这两条和顺着它们的后继（`shelves` 里列出搁置中对）；旁边另一对仍可正常裁决。回 `ingest` 给搁置对所在根追加一笔后继 → 照常入库；`consume` 拉动 → 两侧事件和后继照常放到。再回 `reconcile` **解开搁置** → 这一对才能再裁；早先裁过的对判词不变（不会因解开改判）。重复搁置同一对 → 409，给已裁决的对搁置 → 409，不存在的对 → 404。最后把剩下的并发逐一裁决（A/B 选边），看落库台「导出」从挡住变成全量可导出。
6. **登录 `consume`**：主侧拉动，游标逐次前进到底。
7. **按根暂扣**：在消费台给 `order-100` 按下暂扣 → 再拉动：这根已放到的不再重放、未放到的不再放行，拉动停在它第一条未放出的事件前，别的根（如 `order-200`）照常放行；回 `ingest` 给 `order-100` 追加一笔 → 照常入库（暂扣不挡入库），但导出只带这根已经放到的；回 `consume` 解开暂扣 → 该根从第一条未放出的事件续放，已放到的不改口。落库工/对账工的工位上没有暂扣入口，越权打接口也是 403。
8. 消费台副本区（故障注入按钮只有 `ingest` 能按，消费工按会收到 403——可回 `ingest` 操作）：
...
9. 任意工位底部「权限自检」：所有越权动作都应显示 `403 ✓` / `405 ✓`。
...
GET  /api/holds                      已登录      当前暂扣列表
POST /api/holds                      consume     按根暂扣（根须在库；同根仅一份）
DELETE /api/holds/:root              consume     解开暂扣（已放到的不改口）
GET  /api/export                     ingest      因果齐+已裁决；被扣的根只带已放到的
POST /api/snapshots[/...]            ingest      快照（同根互斥）
GET  /api/queue                      已登录
POST /api/queue/:id/adjudicate       reconcile   裁决（一次定终局）
POST /api/queue/:id/shelve           reconcile   搁置未决对（搁着不能裁；入库/拉动不受影响）
DELETE /api/queue/:id/shelve         reconcile   解开搁置（解开后才能裁；不改已裁结果）
