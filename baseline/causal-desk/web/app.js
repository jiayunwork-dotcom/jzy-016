...
function renderLibrary() {
  ...
  $('#libCounts').textContent =
    `共 ${v.events.length} 条 · 未决并发 ${v.counts.queueOpen} 对（其中搁置 ${v.counts.queueShelved ?? 0} 对）· 已裁 ${v.counts.queueDecided} 对`;

  const released = new Set(v.releasedSeqs || []);
  // 搁置中的并发对两侧事件（搁置期间不导出这两条及后继；入库/拉动照常）
  const shelvedIds = new Set();
  for (const q of v.queue || []) if (q.shelveId) { shelvedIds.add(q.a); shelvedIds.add(q.b); }
  const rows = v.events.map((e) => `
    <tr>
      ...
      <td>${shelvedIds.has(e.id) ? ' <span class="tag shelved" title="所在并发对搁置中：不能选胜负，导出不带；入库/拉动不受影响">搁</span>' : ''}${released.has(e.seq) ? '<span class="tag released" title="已放行给消费者，放出去的不改口">已放到</span>' : (v.holds?.[e.root] ? '<span class="tag heldback" title="暂扣中：还没放到，不再放行；导出也不带">未放出</span>' : '')}<b>${esc(e.id)}</b><div class="muted">${esc(JSON.stringify(e.payload))}</div></td>
...
function renderQueueCounts() {
  const v = state.view;
  if (!v) { $('#queueCounts').textContent = ''; return; }
  const open = v.queue.filter((q) => !q.verdict && !q.shelveId).length;
  const shelved = v.queue.filter((q) => q.shelveId).length;
  const decided = v.queue.filter((q) => q.verdict).length;
  $('#queueCounts').textContent = `待裁决 ${open} 对 · 搁置中 ${shelved} 对 · 已裁 ${decided} 对`;
}
...
$('#runExport').addEventListener('click', async () => {
  try {
    ...
    $('#exportBox').innerHTML = `
      ...
      <div class="muted">未决并发对两侧（含搁置中的）、被暂扣根上还没放到的、以及它们的后继都不带出去；已经放到的不改口。</div>
      ${r.shelves?.length ? `<div class="muted">搁置中 ${r.shelves.length} 对（导出照旧不带这两条及顺着它们的后继）：${r.shelves.map((g) => `${esc(g.queueId)}@${esc(g.root)}（${esc(g.shelveId)}）`).join('、')}</div>` : ''}
...
function renderReconcile() {
  const v = state.view;
  if (!v) return;
  const pending = v.queue.filter((q) => !q.verdict && !q.shelveId);
  const shelved = v.queue.filter((q) => !q.verdict && q.shelveId);
  const decided = v.queue.filter((q) => q.verdict);
  const item = (q) => {
    const a = v.events.find((e) => e.id === q.a);
    const b = v.events.find((e) => e.id === q.b);
    return `<div class="queue-item ${q.verdict ? 'decided' : ''} ${q.shelveId ? 'shelved' : ''}" data-q="${q.id}">
      <div><span class="tag ${q.verdict ? 'decided' : q.shelveId ? 'shelved' : 'open'}">${q.verdict ? '已裁决' : q.shelveId ? '搁置中' : '未决'}</span>
      ...
      <div class="pair">
        <div class="side"><b>A · ${esc(q.a)}</b>
          ...
          ${q.shelveId ? '<div class="shelve-line">搁置中：不能选胜负；导出照旧不带这条及其后继</div>' : ''}
        </div>
      ...
      ${q.verdict
        ...
        : q.shelveId
          ? `<div class="muted">${esc(q.shelvedByName)} 于 ${fmtTime(q.shelvedAt)} 搁置（${esc(q.shelveId)}）：入库与拉动不受影响，解开后才能再裁这一对</div>
             <div class="actions">
               <button class="unshelve" data-unshelve="1">解开搁置（之后才能裁这一对）</button>
             </div>`
          : `<div class="actions">
               <button class="ok" data-win="a">判 A 为胜（${esc(q.a)}）</button>
               <button class="warn" data-win="b">判 B 为胜（${esc(q.b)}）</button>
               <button class="shelve" data-shelve="1">搁置这一对</button>
             </div>`}
    </div>`;
  };
  $('#queueList').innerHTML =
    (pending.length ? '<h3>待裁决</h3>' + pending.map(item).join('') : '') +
    (shelved.length ? '<h3>搁置中（不能选胜负；解开后才能裁；入库/拉动不受影响）</h3>' + shelved.map(item).join('') : '') +
    (!pending.length && !shelved.length ? '<div class="muted">没有未决并发</div>' : '') +
  ...
  $$('#queueList button[data-shelve]').forEach((b) =>
    b.addEventListener('click', async () => {
      const qid = b.closest('.queue-item').dataset.q;
      try {
        const r = await P('POST', `/api/queue/${qid}/shelve`, {});
        toast(`并发对 ${qid} 已搁置（${r.queue.shelveId}）：不能选胜负；入库、拉动不受影响；导出照旧不带这两条及后继`);
        await refreshAll();
        setTimeout(refreshReplicaStatus, 500);
      } catch (e) { toast(e.message, 'err'); }
    })
  );
  $$('#queueList button[data-unshelve]').forEach((b) =>
    b.addEventListener('click', async () => {
      const qid = b.closest('.queue-item').dataset.q;
      try {
        await P('DELETE', `/api/queue/${qid}/shelve`);
        toast(`并发对 ${qid} 已解开搁置，可以再裁；已裁过的对不会因解开而改判`);
        await refreshAll();
        setTimeout(refreshReplicaStatus, 500);
      } catch (e) { toast(e.message, 'err'); }
    })
...
const PROBES = [
  {
    ...
    calls: [
      ...
      { title: '落库工搁置并发对（POST /api/queue/q1/shelve）', run: () => P('POST', '/api/queue/q1/shelve', {}), expect: 403 },
      { title: '落库工解开搁置（DELETE /api/queue/q1/shelve）', run: () => P('DELETE', '/api/queue/q1/shelve'), expect: 403 },
  ...
  },
  {
    ...
    calls: [
      ...
      { title: '消费工搁置并发对（POST /api/queue/q1/shelve）', run: () => P('POST', '/api/queue/q1/shelve', {}), expect: 403 },
      { title: '消费工解开搁置（DELETE /api/queue/q1/shelve）', run: () => P('DELETE', '/api/queue/q1/shelve'), expect: 403 },
  ...
  },
  {
    ...
    calls: [
      ...
      { title: '给已裁决的对搁置（POST shelve）', run: async () => {
          const q = state.view?.queue.find((x) => x.verdict);
          if (!q) return { status: 0 };
          return P('POST', `/api/queue/${q.id}/shelve`, {});
        }, expect: 409 },
      { title: '搁置不存在的对（POST shelve）', run: () => P('POST', '/api/queue/q-not-exist/shelve', {}), expect: 404 },
      { title: '解开没搁置的对（DELETE shelve）', run: () => P('DELETE', '/api/queue/q-not-exist/shelve'), expect: 404 },
      { title: '副本上搁置并发对（POST :4001 只读）', run: () => R('POST', '/api/queue/q1/shelve', {}), expect: 405 }
    ]
  }
];
